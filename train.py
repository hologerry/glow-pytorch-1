        ...
        for i in pbar:
            ...
            if i % args.sample_freq == 0:
                with torch.no_grad():
                    ...
                    # sample at first
                    utils.save_image(
                        model_single.reverse(z_sample_first)[1].cpu().data,
                    ...
                    # sample
                    utils.save_image(
                        model_single.reverse(z_sample)[1].cpu().data,
                    ...
                    # reverse
                    utils.save_image(
                        model_single.reverse(z_sample, reconstruct=True)[1].cpu().data,
                    ...
                    # reconstruct
                    utils.save_image(
                        model_single.reverse(z_encode_t, reconstruct=True)[1].cpu().data,
                        os.path.join(log_dir, 'sample', f'{str(i).zfill(6)}_target_reconstruct.png'),
                        normalize=True,
                        nrow=args.batch//4,
                        range=(-0.5, 0.5),
                    )
                    ...
                    utils.save_image(
                        image.cpu().data,
                        os.path.join(log_dir, 'sample', f'{str(i).zfill(6)}_target_ground-truth.png'),
                        normalize=True,
                        nrow=args.batch//4,
                        range=(-0.5, 0.5),
                    )
            ...
            if i % args.check_freq == 0:
                ...
                torch.save(
                    encoder.state_dict(), os.path.join(log_dir, 'checkpoint', f'encoder_{str(i).zfill(6)}.pt')
                )
                torch.save(
                    encoder.state_dict(), os.path.join(log_dir, 'checkpoint', 'encoder_latest.pt')
                )
                torch.save(
